    ...
    return pd.DataFrame(
        {
            ...
            "calificacion_calidad": np.round(
                rng.uniform(2.5,5.0,size=row_count,),
                2,
            ),
            "activo": rng.choice([True, False],size=row_count,p=[0.95, 0.05],
            ),
        }
    )
...
def generate_articulos(
    ...
    selected_categories = [categories[index]for index in rng.integers(0,len(categories),size=row_count,)
    ]
    ...
    category_lines = [
        ...
        for subcategory, line in zip(subcategories,rng.integers(1,6,size=row_count,),strict=True,)
    ]

    # Distribución con mayor concentración
    # de artículos en precios bajos y medios.
    prices = np.round(
        np.clip(rng.lognormal(mean=np.log(200),sigma=1.0,size=row_count,
            ),
            10.00, 5000.00,
        ),
        2,
    ...
    return pd.DataFrame(
        {
            ...
            "cod_barra": [fake.unique.ean13()
                for _ in range(row_count)
            ],
            "desc_art": [
                (f"{subcategory} " f"{fake.word().capitalize()}"
                )
            ...
            "precio_lista": prices,
            "peso_kg": np.round(
                rng.uniform( 0.10, 20.00, size=row_count,
                ),
                2,
            ),
    ...
    return pd.DataFrame(
        {
            ...
            "nom_tienda": [
                ...
                for store_id, location in enumerate(
                    selected_locations,
                    start=1,
                )
            ],
            "tipo_tienda": rng.choice(
                reference_data["store_types"],
                size=row_count,
            ),
            "id_ciudad": [
                location["id_ciudad"]
    ...
    return pd.DataFrame(
        {
            ...
            "fec_registro": generate_random_dates(
                row_count=row_count,
                start_date=(
                    start_date
            ...
            "genero": rng.choice(
                reference_data["genders"],
                size=row_count,
            ),
            "rango_edad": generate_age_groups(
                row_count=row_count,
                rng=rng,
            ),
...
def generate_ventas(
    ...
    channels = rng.choice(
        reference_data["sales_channels"],
        size=row_count,
    )

    payment_types = np.empty(
        row_count,
        dtype=object,
    )

    # Cada canal utiliza únicamente
    # los medios de pago configurados.
    for channel in np.unique(channels):
        channel_mask = channels == channel

        payment_types[channel_mask] = (
            rng.choice(
                reference_data[
                    "payment_types"
                ][str(channel)],
    ...
    return pd.DataFrame(
        {
            ...
            "hra_trans": generate_sales_times(
                row_count=row_count,
                rng=rng,
            ),
...
def generate_stock(
    ...
    """
    Genera la tabla INV_STOCK_DIARIO.

    Se crea una combinación única
    de cada artículo con cada tienda.
    """
    expected_rows = (
        len(articulos)
        * len(tiendas)
    ...
    if row_count != expected_rows:
        raise ValueError(
            "El volumen de stock debe ser "
            "igual a artículos × tiendas. "
            f"Configurado: {row_count:,}; "
            f"esperado: {expected_rows:,}."
        )

    minimum_stock = rng.integers(
        5,
        81,
        size=row_count,
    )
...
def generate_devoluciones(
    ...
    """
    Genera la tabla POST_DEVOLUCIONES
    utilizando ventas existentes.
    """
    ...
    statuses = rng.choice(
        reference_data["return_statuses"],
        size=row_count,
    )
...
def generate_source_data(
    ...
    """
    Genera las siete tablas fuente
    del escenario RetailMax.
    """
...
if __name__ == "__main__":
    ...
    duplicate_stock_keys = (
        stock.duplicated(
            subset=["art_id","id_tienda",]).sum()
    )
    assert duplicate_stock_keys == 0

    print("\nVerificación de inventario:")
    print("Combinaciones artículo-tienda:", f"{len(stock):,}",)
    print("Combinaciones duplicadas:",duplicate_stock_keys,)
