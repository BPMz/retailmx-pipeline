...
def _run_self_test() -> None:
    ...
    ventas = pd.DataFrame(
        {
            "id_trans": np.arange(1, 101),
            "fec_trans": pd.date_range(
                "2024-01-01",
                periods=100,
            ),
            "qty_vendida": np.ones(
    ...
    devoluciones = pd.DataFrame(
        {
            "id_devolucion": np.arange(1, 21),
            "id_trans_origen": np.arange(1, 21),
            "qty_devuelta": np.ones(
                20,
                dtype=int,
            ),
            "estado_devolucion": [
    ...
    config = {
        ...
        "anomalies": {
            "enabled": True,
            "duplicate_sales_rate": 0.05,
            "out_of_range_dates_rate": 0.04,
            "inconsistent_returns_rate": 0.10,
        },
    }

    tables, report = inject_anomalies(
        tables={
            "TRANS_VENTAS": ventas,
    ...
    assert report["affected_rows"].tolist() == [5,4,2,]

    business_columns = ventas.columns.drop("id_trans")

    assert ventas.duplicated(subset=business_columns).sum() >= 5

    assert (pd.to_datetime(tables["TRANS_VENTAS"]["fec_trans"])
    ...
    print("\nPrueba de anomalies.py ""completada correctamente.")
